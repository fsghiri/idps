...
static Flow flow __attribute__((section(".SRAM2")));
...
void nfem_finalize(float feat[IEC104_FEATURE_COUNT])
{
    ...
    /* guarantee non-empty stats so features are always finite */
    ...
    /* fill feature vector */
    feat[0]  = (float)(1e-6 * stat_max(&flow.idle_time));
    feat[1]  = (float)(1e-6 * stat_min(&flow.idle_time));
    feat[2]  = (float)(1e-6 * flow.idle_time.mean);
    feat[3]  = (float)(1e-6 * stat_var(&flow.idle_time));
    feat[4]  = (float)(1e-6 * stat_min(&flow.active_time));
    feat[5]  = (float)(1e-6 * flow.active_time.mean);
    feat[6]  = (float)(1e-6 * stat_std(&flow.active_time));
    feat[7]  = (float)(1e-6 * stat_var(&flow.active_time));
    feat[8]  = (float)(1e-6 * stat_min(&flow.iat_bw));
